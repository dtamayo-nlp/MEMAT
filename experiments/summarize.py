...
def find_correct_indices(
    ...
    for run_dir in (RESULTS_DIR / dir_name).iterdir():
        if run not in str(run_dir):
            continue
        ...
        for k, case_file in enumerate(files):
            try:
                with open(case_file, "r") as f:
                    data = json.load(f)
            except:
                print(f"Could not decode {case_file}!!")
            ...
            prefix = "post"
            key = f"rewrite_prompts_correct"
            correct_samples.append(data[prefix][key])
...
def main(
    ...
    for run_dir in (RESULTS_DIR / dir_name if not abs_path else dir_name).iterdir():
        ...
        if apply_std_norm:
            cur_sum = {k: (np.mean(v), 1.96*np.std(v)/np.sqrt(number_norm)) for k, v in cur_sum.items()}
        else:
...
def summary(
    ...
    for run_dir in (RESULTS_DIR / dir_name if not abs_path else dir_name).iterdir():
        ...
        for k, case_file in enumerate(files):
            ...
            if offset is not None and case_id < offset[0]:
                continue
            elif offset is not None and case_id > offset[1]:
                break
...
if __name__ == "__main__":
    ...
    print("Remember, the difference between accuracy and success is that accuracy measures whether the most probable token is target_new, and success measures whether p(target_new)>p(target_true).")
    print("---------")
    print("Also note that the logits in eval_utils_counterfact are with a negative sign, which changes some choices in > and <.")
