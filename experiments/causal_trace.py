...
def layername(model, num, kind=None, modelname = "aguila"):
    if modelname == "aguila":
        if kind == "embed":
...
def plot_all_flow(mt, prompt, subject=None):
    for kind in ["mlp", "self_attention", None]:
...
def make_inputs(tokenizer, prompts, device = "cuda"):
    ...
    return tokenizer(prompts, padding = True, truncation = True, return_tensors = "pt").to(device)
