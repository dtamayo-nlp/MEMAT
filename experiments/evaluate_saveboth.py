...
def main(
    ...
    if continue_from_run is None:
        ...
        catalan_dir = run_dir / "catalan"
        english_dir = run_dir / "english"
        
        run_dir.mkdir(parents=True, exist_ok=True)
        catalan_dir.mkdir(parents=True, exist_ok=True)
    ...
    # We will load both datasets to make the prediction
    if language in "catalan_CF.json":
        ds_2 = ds_class(DATA_DIR, "english_CF.json", tok=tok, size=dataset_size_limit)
        other_lang = "english"

    elif language in "english_CF.json":
    ...
    for num_c, record_chunks in enumerate(chunks(ds, num_edits)):
        ...
        for lang in [language,other_lang]:
            for idx in range(num_edits):

                if lang == language:
                    record = ds[num_c*num_edits+idx]
                else:
                ...
                out_file = Path(case_result_template.format(lang,num_edits, record["case_id"]))
                if out_file.exists():
