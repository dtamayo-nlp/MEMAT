...
class SecondMoment(Stat):
    ...
    def add(self, a):
        a = self._normalize_add_shape(a).detach().cpu()
        if len(a) == 0:
            return
        # Initial batch reveals the shape of the data.
        if self.count == 0:
            self.mom2 = a.new(a.shape[1], a.shape[1]).zero_().detach().cpu().double()

        batch_count = a.shape[0]
        # Update the covariance using the batch deviation
        self.count += batch_count
        curr_operation = a.double().t().mm(a.double())
        # print(a[:,2226])
        # print("coordinate -------------->",a[:,2224])
        self.mom2 += curr_operation
