...
def generate_mine(
    ...
    outputs = []
    for text in prompts:
        outputs.append(pipeline(text)[0]["generated_text"])
    return outputs
...
def generate_fast(
    ...
    top_k: int = 5,
    max_out_len: int = 200,
): # FOR SOME REASON THIS FUNCTION FAILS HEAVILY with the base parameter itself
    """
    ...
    with torch.no_grad():
        while input_ids.size(1) < max_out_len:  # while not exceeding max output length
            ...
            last_non_masked = attention_mask.sum(1) - 1 # Index of the last index token of the prompt, that is why we are substracting one
            for i in range(batch_size):
                new_idx = last_non_masked[i] + 1 # Now we are adding one because we want the index of the next token!!
